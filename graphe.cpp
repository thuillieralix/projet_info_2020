...
//void Graphe::Dijkstra_intermediarite(int depart, int arrivee)
//{
//        std::vector<bool> marque;
//        for(size_t i=0; i<m_sommet.size(); ++i)    ///initialisation du vecteur qui détermmine si on est bien passé sur un sommet
...
//            if(i==depart)
//            {
//              preds.push_back(depart);
//            }else preds.push_back(100);
//        }
//
...
//        int temp = 0;
//        marque[depart]=true;
//        int nb_poids_arrete=0;
//        for(size_t i =0; i < m_sommet[depart]->getAdj().size();++i)
//        {
//            for (size_t j=0;j<m_arrete.size();j++)
//            {
//                Sommet* s=m_sommet[depart]->getAdj()[i];
//                if(m_arrete[j]->getDepart()==s->getIndice()&&m_sommet[depart]->getIndice()==m_arrete[j]->getArrivee()
//                            ||m_arrete[j]->getArrivee()==s->getIndice()&&m_sommet[depart]->getIndice()==m_arrete[j]->getDepart())
//                {
//                    nb_poids_arrete=m_arrete[j]->getPoids();
//                }
//            }
//            int nb;
//            //nb=m_sommet[depart]->getAdj()[i]->getPoids()
//            liste.push(nb_poids_arrete);
//            distance[m_sommet[depart]->getAdj()[i]->getNum()] = m_sommet[depart]->getDist()[i];
//            preds[m_sommet[depart]->getAdj()[i]->getNum()] = depart;
//        }
//        while (fin != true)
//        {
//            fin = true;
...
//                {
//                    //liste.push(m_sommets[temp]->getDist()[i] + distance[temp]);
//                    liste.push(m_sommets[temp]->getAdj()[i]->getPoids() + distance[temp]);
//                    distance[m_sommet[temp]->getAdj()[i]->getNum()] = m_sommet[temp]->getDist()[i] + distance[temp];
//                    preds[m_sommet[temp]->getAdj()[i]->getNum()] = temp;
//                }
//            }
//            liste.pop();
//        }
